...
def test_parse_frames_count():
    ...
    parse_frames() should return 4-element tuples (file, line, function, module)
    so that callers can filter by module without parsing the file path themselves.

    Root cause: the regex only captures three groups (file, line, function);
    it does not extract the top-level module from the file path.
    """
    fp = CrashFingerprint(FULL_TRACE, "ValueError", "Invalid input")
...
def test_same_crash_different_ip_gets_same_fingerprint():
    ...
    Two crashes caused by the same code path but connecting to different
    database hosts should produce the *same* fingerprint — they are the
    same bug, just observed on different hosts.

    Root cause: generate() includes the raw error_message in the hash
    without normalisation, so 'Connection refused: 10.0.0.1' and
...
def test_normalize_strips_uuid():
    ...
    normalize_message() should strip UUIDs so that 'Entity <uuid> not found'
    errors group together regardless of which entity triggered the crash.
